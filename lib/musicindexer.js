...
export function MusicIndexer({fileCachePath, mlCachePath, waPath = null} = {}) {
  ...
  /**
   * Reformats the playlist data to be more like the category data.
   */
  function formatPlaylistData(playlist) {
    return {
      name: playlist.title,
      code: playlist.title.toLowerCase().replace(/ /g, '_'),
      type: 'playlist',
      isEndNode: true,
      items: Object.fromEntries(
        playlist.tracks
          .map(track => state.data[track.file])
