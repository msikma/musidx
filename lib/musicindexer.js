...
export function MusicIndexer({fileCachePath, mlCachePath} = {}) {
  const state = {
    data: null,
    ml: null,
  ...
  async function indexPath(musicPath, {categories, onlyCategories = false, forceRefresh = false, printSpeed = false} = {}) {
    state.data = await readCache(fileCachePath)
    if (!onlyCategories) {
      const files = await findFiles(musicPath)
      await processFiles(musicPath, files, categories, {forceRefresh, printSpeed})
      await checkFileExistence(musicPath)
      await writeCache(fileCachePath, state.data)
    }
    await catalogueMediaLibrary(categories)
